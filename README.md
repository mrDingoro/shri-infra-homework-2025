...
Проект использует [Semantic Versioning](https://semver.org/spec/v2.0.0.html) в формате `MAJOR.MINOR.PATCH`:

| Тип | Формат | Когда использовать | Пример |
|-----|--------|-------------------|---------|
| **🏗️ Новый релиз** | `v1.0.0` | Новая функциональность, исправления | `v1.0.0`, `v1.1.0`, `v2.0.0` |
| **🔧 Хотфикс** | `v1.0.0-hotfix.1` | Критические баги в релизе | `v1.0.0-hotfix.1`, `v1.0.0-hotfix.2` |
...
2. **2. 🏗️ Создание релиза (ручной запуск)** (`release.yml`)
   - **Цель:** Подготовить стабильную версию для выкатки
   - Запускается вручную через GitHub Actions UI с указанием версии (например: `1.0.0`)
   - Валидирует формат версии (semantic versioning)
   - Создает релизную ветку `releases/v<версия>` (например: `releases/v1.0.0`)
   - Обновляет версию в `package.json`
   - Собирает и загружает Docker образы с тегами:
     - `app:<версия>` (например: `app:1.0.0`)
     - `app:<версия>-latest` (например: `app:1.0.0-latest`)
     - `app:latest`
   - Создает Git тег `v<версия>` и GitHub Release
   - Создает GitHub Issue с информацией о релизе
   - Обновляет CHANGELOG.md в релизной ветке

3. **3. 🚀 Деплой в продакшн (ручной запуск)** (`deploy.yml`)
   - **Цель:** Выкатить готовый релиз к пользователям
   - Запускается **после создания релиза ИЛИ хотфикса** с указанием версии (например: `1.0.0`)
   - Валидирует формат версии
   - Проверяет существование Docker образа `app:<версия>-latest` в registry
   - Деплоит приложение на VM через SSH
   - Добавляет переменную окружения `APP_VERSION` в контейнер
   - Создает GitHub Deployment
   - Обновляет Issue с информацией о деплое и добавляет лейбл `deployed`

4. **4. 🔧 Хотфикс → затем запустить "3. 🚀 Деплой" (ручной запуск)** (`hotfix.yml`)
   - **Цель:** Срочно исправить баги в уже задеплоенной версии
   - Запускается **после деплоя** когда найдены критические ошибки
   - Принимает версию базового релиза (например: `1.0.0`)
   - Работает с релизными ветками `releases/v<версия>`
   - Автоматически вычисляет номер хотфикса (например: `1.0.0-hotfix.1`)
   - Выполняет линтер, проверку типов и тесты параллельно
   - Обновляет версию в `package.json`
   - Создает образы с тегами:
     - `app:<версия>-hotfix.<номер>` (например: `app:1.0.0-hotfix.1`)
     - `app:<версия>-latest` (обновляет latest для данной версии)
   - Создает Git тег `v<версия>-hotfix.<номер>` и GitHub Release
   - ⚠️ **После хотфикса ОБЯЗАТЕЛЬНО запустить "3. 🚀 Деплой"**
   - Добавляет комментарий в соответствующий Issue с лейблом хотфикса
